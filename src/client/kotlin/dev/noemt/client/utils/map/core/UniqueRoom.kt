...
class UniqueRoom(arrX: Int, arrY: Int, roomTile: RoomTile) {
    ...
    fun updateBounds(roomSize: Float, gapSize: Float) {
        ...
        if (data.shape != RoomShape.SL && data.shape != RoomShape.S1x1 && data.shape != RoomShape.S2x2) {
            val gridPositions = tiles.map { it.getGridPos() }
            val minCol = gridPositions.minOfOrNull { it.second } ?: 0
            val maxCol = gridPositions.maxOfOrNull { it.second } ?: 0
            val minRow = gridPositions.minOfOrNull { it.first } ?: 0
            val maxRow = gridPositions.maxOfOrNull { it.first } ?: 0

            val tilesWide = (maxCol - minCol) / 2 + 1
            val tilesTall = (maxRow - minRow) / 2 + 1
    ...
    fun addTile(x: Int, y: Int, tile: RoomTile) {
        ...
        val roomTiles = tiles.mapNotNull { t -> if (t.isSeparator) null else t.getGridPos() }.ifEmpty { return }
        val xRooms = roomTiles.groupBy { it.second }.entries.sortedByDescending { it.value.size }
        val zRooms = roomTiles.groupBy { it.first }.entries.sortedByDescending { it.value.size }
    ...
    fun getCheckmarkPosition() = if (ConfigManager.config.map.centerStyle) center else topLeft
    ...
    fun findRotation() {
        ...
        val highestBlock = highestBlock ?: ScanUtils.getHighestY(mainRoom.x, mainRoom.z).takeIf { it > 0 }?.also { highestBlock = it } ?: return
