...
object ScoreCalculation {
        ...
        EventBus.register<MainThreadPacketReceivedEvent.Post> {
            ...
            when (val packet = event.packet) {
                is ClientboundPlayerInfoUpdatePacket -> {
                    if (ADD_PLAYER !in packet.actions() && UPDATE_DISPLAY_NAME !in packet.actions()) return@register
                ...
                is ClientboundEntityEventPacket -> {
                    ...
                    val entity = mc.level?.let { packet.getEntity(it) }
                    if ((entity as? Zombie)?.isBaby != true) return@register
                    mimicKilled = true
                }
                ...
                is ClientboundSystemChatPacket -> {
                    ...
                    if (!princeKilled) {
                        if (msg == "a prince falls. +1 bonus score" || princeMessages.any { msg.contains(it) }) {
                            princeKilled = true
                        }
                    }

                    if (!mimicKilled && (LocationUtils.dungeonFloorNumber ?: 0) > 5 && !LocationUtils.inBoss) {
                        if (mimicCharmRegex.matches(msg) || msg == "charm you charmed a mimic and captured its shard." || mimicMessages.any { msg.contains(it) }) {
                            mimicKilled = true
                        }
                    }

                    if (!batKilled) {
                        if (msg == "a bat has been slain. +1 bonus score" || batMessages.any { msg.contains(it) }) {
                            batKilled = true
                        }
                    }
                }
    ...
    private fun recalculate() {
        ...
        val puzzlePenalty = DungeonListener.puzzles.count { it.state != RoomState.GREEN && it.state != RoomState.CLEARED } * 10
