...
import dev.noemt.client.utils.MathUtils.add
    ...
    override fun init() {
        register<ChatMessageEvent> {
            ...
            if (config.killTitle) {
                ...
                ChatUtils.modMessage("Watcher will move in ${moveSeconds.toDouble().toFixed(2)}s.")
                ThreadUtils.scheduledTask(moveTicks) {
                    ChatUtils.showTitle("&c&lKill Mobs")
                }
            }

            if (!config.speedAlert) return@register
            ...
            mc.execute {
                ...
                if (config.partySpeedAlert) ChatUtils.sendPartyMessage(title)
